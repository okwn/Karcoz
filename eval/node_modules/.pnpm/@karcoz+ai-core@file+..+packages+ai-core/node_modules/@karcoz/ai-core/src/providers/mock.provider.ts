import type {
  ...
  PracticeGenerationInput, GeneratedQuestion, ProviderName,
} from '../types.js';
...
export class MockProvider implements AIProvider {
  ...
  setMockDelay(ms: number) { this.delay = ms; }
  ...
  async extractQuestion(input: ExtractionInput): Promise<QuestionExtraction> {
    ...
    if (!text || text.length < 5) throw new NoQuestionDetectedError(this.name);
  ...
  async validateAnswer(input: ValidationInput): Promise<ValidationResult> {
    ...
    const issues: string[] = [];
    let adj = 0;
    if (!input.shortAnswer?.trim()) { issues.push('Empty answer'); adj = -0.3; }
  ...
  async generatePractice(input: PracticeGenerationInput): Promise<GeneratedQuestion[]> {
  ...
  private lang(text: string): 'en' | 'tr' | 'unknown' {
    return /[şçğüöı]/i.test(text) ? 'tr' : 'en';
  }

  private qType(text: string) {
    if (/\n[A-D]\.\s/.test(text)) return 'multiple_choice';
    if (/true|false/i.test(text)) return 'true_false';
    return 'short_answer';
  }

  private answer(q: string): string {
    if (q.includes('capital') || q.includes('France')) return 'Paris';
    if (q.includes('derivative')) return '2x + 3';
    return 'See explanation';
  }

  private explain(q: string, level: string): string {
    const base = 'Based on analysis of the question: ';
    if (level === 'brief') return base + 'Answer determined.';
    if (level === 'detailed') return base + 'Detailed step-by-step reasoning. Tests core concept understanding.';
