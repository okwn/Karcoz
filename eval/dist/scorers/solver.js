...
// ── OCR scoring ───────────────────────────────────────────────────────────────
export function scoreOcr(entry, extractedText, _normalizedText) {
    ...
    const normalizedTextSimilarity = union.size > 0 ? intersection.size / union.size : 1;
    // Raw text exact match
    const rawTextMatch = extractedText.toLowerCase().trim() === expected.toLowerCase().trim();
    return {
        rawTextMatch,
...
export function scoreOptions(entry, extractedOptions) {
    ...
    for (const exp of expected) {
        ...
        const found = extractedOptions?.find((opt) => {
            ...
            const valueSim = levenshtein(opt.value.toLowerCase(), expValue.toLowerCase()) /
...
// ── Topic scoring ──────────────────────────────────────────────────────────────
export function scoreTopic(entry, actualTopic) {
    ...
    const topicMatch = actual.length > 0 && (actual.includes(expected) || expected.includes(actual));
    return {
        topicMatch,
...
// ── Answer scoring ─────────────────────────────────────────────────────────────
export function scoreAnswer(entry, shortAnswer, selectedOption) {
    ...
    if (labelOptions.includes(correct)) {
        ...
        const textMatch = correctOptionText.length > 0 &&
        ...
        const similarity = 1 - levenshtein(given.toLowerCase(), correct.toLowerCase()) / Math.max(given.length, correct.length, 1);
        return {
...
// ── Confidence scoring ─────────────────────────────────────────────────────────
export function scoreConfidence(confidenceScore, isCorrect) {
...
export function scoreLatency(perf, hasImage) {
...
// ── Explanation scoring ─────────────────────────────────────────────────────────
export function scoreExplanation(fullExplanation) {
...
export function assembleQuestionResult(entry, opts) {
    const passedDimensions = [];
    const failedDimensions = [];
    if (opts.ocrScore) {
        const dim = 'ocr';
        if (opts.ocrScore.normalizedTextSimilarity > 0.7)
    ...
    if (opts.optionParseScore) {
        const dim = 'options';
        if (opts.optionParseScore.optionAccuracy >= 0.75)
            passedDimensions.push(dim);
        else
            failedDimensions.push(dim);
    }
    ...
    {
        const dim = 'answer';
        if (opts.answerScore.exactMatch)
            passedDimensions.push(dim);
        else
            failedDimensions.push(dim);
    }
    if (opts.confidenceScore) {
        ...
        if (opts.confidenceScore.isWellCalibrated)
            passedDimensions.push(dim);
        else
            failedDimensions.push(dim);
    }
    {
        const dim = 'latency';
        if (opts.latencyScore.withinSla)
            passedDimensions.push(dim);
        else
            failedDimensions.push(dim);
    }
    if (opts.explanationScore) {
        ...
        if (opts.explanationScore.hasExplanation)
            passedDimensions.push(dim);
        else
            failedDimensions.push(dim);
    }
