...
export function initHybridEngine(config: HybridOCRConfig): void {
  ...
  if (config.visionProvider) {
    ...
    visionEngine = new VisionOCREngine(config.visionProvider as any);
  }
 ...
 */
async function applyPreprocessingSteps(
  image: Buffer | string
): Promise<{ buffer: Buffer | string; steps: string[] }> {
  const steps: string[] = [];
...
export async function hybridOCR(input: OCRInput): Promise<OCRResult> {
  ...
  let preprocessedImage: Buffer | string | undefined;
