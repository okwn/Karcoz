...
export class OpenRouterProvider implements AIProvider {
  ...
  constructor(config: string | OpenRouterConfig) {
    if (typeof config === 'string') {
      ...
      if (!config.apiKey) throw new AIProviderError('OPENROUTER_API_KEY is required', 'MISSING_API_KEY', 'openrouter');
  ...
  private async makeRequest(payload: Record<string, unknown>): Promise<{ choices?: Array<{ message?: { content?: string } }> }> {
      ...
      if (!response.ok) {
        ...
        if (response.status === 429) {
          throw new AIProviderError(
            `OpenRouter rate limit: ${response.status}`,
            'RATE_LIMITED',
            'openrouter',
            429,
            errorBody
  ...
  private parseValidationResponse(content: string): ValidationResult {
    const cleaned = this.extractJSON(content);
    if (!cleaned) return { status: 'not_validated', issues: ['Could not parse validation response'], confidenceAdjustment: 0 };
  ...
  private parseTopicResponse(content: string): TopicClassification {
    const cleaned = this.extractJSON(content);
    if (!cleaned) return { topic: 'General', confidence: 0.5 };
