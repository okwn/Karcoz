...
export class OpenAIProvider {
    constructor(config) {
        ...
        if (typeof config === 'string') {
            ...
            if (!config.apiKey)
                throw new AIProviderError('OPENAI_API_KEY is required', 'MISSING_API_KEY', 'openai');
    ...
    async extractQuestion(input) {
        ...
        const userParts = ['Extract the question and any choices from this image.'];
        ...
        try {
            ...
            if (!response.ok) {
                ...
                if (response.status === 429) {
                    throw new AIProviderError(`OpenAI rate limit: ${response.status}`, 'RATE_LIMITED', 'openai', 429, errorBody);
                }
                throw new AIProviderError(`OpenAI API error: ${response.status} ${response.statusText}`, 'PROVIDER_ERROR', 'openai', response.status, errorBody);
            }
            ...
            if (isRetryableError(err))
                throw err;
            throw new AIProviderError(`OpenAI request failed: ${err instanceof Error ? err.message : String(err)}`, 'PROVIDER_ERROR', 'openai', undefined, err);
        }
    }
    ...
    parseValidationResponse(content, originalAnswer) {
        const cleaned = this.extractJSON(content);
        if (!cleaned) {
