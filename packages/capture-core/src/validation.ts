 ...
 */
export function validateCaptureOptions(options: CaptureOptions): ValidationError | null {
  if (!options.source) {
 ...
 */
export async function captureImage(options: CaptureOptions): Promise<CaptureResult> {
  ...
  if (cropRect) {
    const rawRect = calculateImageCropRect(
      cropRect,
      browserZoom,
      devicePixelRatio
    );
