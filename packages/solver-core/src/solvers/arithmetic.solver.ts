...
import { formatCompactAnswer } from '../explain/compact-answer-formatter.js';
 ...
 */
export function solveArithmetic(question: SolverQuestion): SolverResult {
  ...
  if (equalityMatch) {
    ...
    if (operations[op]) {
      const computed = operations[op]((a as number), (b as number));
