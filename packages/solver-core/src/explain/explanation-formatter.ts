...
export function formatExplanation(
  ...
  switch (solverType) {
    case 'arithmetic': {
      ...
      const verified = d.verified as boolean | undefined;
      if (lang === 'tr') {
    ...
    case 'ratio': {
      ...
      if (lang === 'tr') return `Oran: ${ratio} → eksik: ${value}`;
    ...
    case 'simple_algebra': {
      ...
      const result = String(d.result ?? '');
      const direct = d.direct as boolean | undefined;
      if (lang === 'tr') {
    ...
    case 'sequence': {
      ...
      const typ = String(d.type ?? '');
      if (lang === 'tr') return `${typ} dizi: ${seq} → bir sonraki: ${next}`;
      return `${typ} sequence: ${seq} → next: ${next}`;
    }

    case 'multiple_choice': {
      const count = d.optionCount as number ?? 0;
      if (lang === 'tr') {
 ...
 */
export function briefSummary(solverType: string, result: string, language: 'en' | 'tr' | 'unknown'): string {
  const lang = language === 'unknown' ? 'tr' : language;
