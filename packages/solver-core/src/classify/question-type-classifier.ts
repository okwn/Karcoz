 ...
 */
export function classifyQuestionType(question: SolverQuestion): QuestionTypePrediction {
  ...
  for (const p of sequencePatterns) {
    if (p.test(text)) return { type: 'sequence', confidence: 0.85 };
  }
  ...
  for (const p of algebraPatterns) {
    if (p.test(text)) return { type: 'simple_algebra', confidence: 0.85 };
  }
  ...
  for (const p of percentPatterns) {
    if (p.test(text)) return { type: 'percentage', confidence: 0.9 };
  }
  ...
  for (const p of ratioPatterns) {
    if (p.test(text)) return { type: 'ratio', confidence: 0.8 };
  }
  ...
  for (const p of arithmeticPatterns) {
    if (p.test(text)) return { type: 'arithmetic', confidence: 0.85 };
  }
