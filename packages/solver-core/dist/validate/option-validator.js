 ...
 */
export function validateOption(selectedOption, shortAnswer, options) {
    const issues = [];
    let adjustment = 0;
    if (selectedOption === undefined) {
        // No option selected - check if answer text matches any option
        const matchedIndex = options.findIndex(o => o.value.toLowerCase().trim() === shortAnswer.toLowerCase().trim() ||
