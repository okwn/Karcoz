...
class KarcozTelegramBot {
  ...
  constructor(botToken: string, appBaseUrl: string = 'http://localhost:3100') {
  ...
  private async handleCommand(command: string, chatId: string, username?: string) {
    switch (command) {
      case 'start':
  ...
  private async handlePhoto(chatId: string, photo: Array<{ file_id: string }>, caption?: string, username?: string) {
    ...
    try {
      ...
      if (!fileData.ok || !fileData.result) throw new Error('Could not get file');
      ...
      // Solve via API
      const result = await solveService.solveFromImage(
        { imageBase64: base64, sourceType: 'upload', pageTitle: `Telegram:${chatId}`, explanationLevel: 'standard', resultMode: 'full' },
        { requestId: `tg_${Date.now()}`, ipAddress: 'telegram', userAgent: 'telegram-bot' }
      );
  ...
  private async handleLink(chatId: string, username?: string) {
    const account = await telegramService.getAccountByChatId(chatId);
  ...
  private async handleUnlink(chatId: string) {
    const account = await telegramService.getAccountByChatId(chatId);
