...
export function hasFeature(plan: PlanName, feature: FeatureName): boolean {
  const limits = getPlanLimits(plan);
  const value = limits.features[feature];
  return typeof value === 'boolean' ? value : true;
}

export function getSolveLimit(plan: PlanName, period: 'daily' | 'monthly'): number {
  return PLAN_LIMITS[plan]?.solve[period] ?? PLAN_LIMITS.free.solve[period];
}

export function getImageMaxBytes(plan: PlanName): number {
