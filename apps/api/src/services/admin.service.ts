...
export class AdminService {
  ...
  async getAdminOverview() {
    ...
    const [userCount, questionCount, todaySolveCount, errorCount, recentQuestions] =
      await Promise.all([
  ...
  async getSolveLatencyStats(limit: number = 1000) {
    ...
    const latencies = recentLogs
      .map(log => {
        ...
        return typeof d?.latencyMs === 'number' ? (d.latencyMs as number) : null;
      })
  ...
  async getErrorStats() {
    ...
    for (const log of recentErrors) {
      const d = log.details as Record<string, unknown> | null;
      const code = (d?.errorCode as string) ?? 'UNKNOWN';
  ...
  async updateModelConfig(
    ...
    return {
      provider: updated.provider as ProviderName,
      modelName: updated.modelName,
  ...
  async updateRateLimit(
    adminId: string,
    tier: string,
    patch: { minute?: number; daily?: number; monthly?: number }
  ) {
