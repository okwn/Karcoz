import { getDefaultProviderName, getConfiguredProviderName, globalRegistry } from '@karcoz/ai-core';
...
function getProviderWithFallback(): ProviderWithFallback {
  ...
  const primary = (() => {
    ...
    const created = (globalRegistry as unknown as {
      getConfiguredProvider: (n: string) => AIProvider;
    }).getConfiguredProvider(primaryName);
...
export async function extractFromImage(imageBase64: string): Promise<ExtractionResult> {
...
export async function extractFromText(text: string): Promise<ExtractionResult> {
  ...
  const questionType: ExtractionResult['questionType'] = isMultipleChoice ? 'multiple_choice' : 'short_answer';
  ...
  let result: Awaited<ReturnType<AIProvider['extractQuestion']>> | undefined;
...
function extractOptions(text: string): { label: string; value: string; order: number }[] {
  const options: { label: string; value: string; order: number }[] = [];
