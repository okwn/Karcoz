...
import { createUsageLimitService, type UsageEventType } from '../services/usage-limit.service.js';
 ...
 */
const ENDPOINT_TO_EVENT: Record<string, UsageEventType> = {
  '/api/solve/image': 'solve_image',
  '/api/solve/text': 'solve',
  '/api/practice/generate': 'practice_generate',
};

function mapEndpointToEvent(url: string): UsageEventType | null {
  for (const [pattern, eventType] of Object.entries(ENDPOINT_TO_EVENT)) {
    if (url.startsWith(pattern)) return eventType;
  }
  return null;
}

export function registerRateLimitMiddleware(app: FastifyInstance, prisma: PrismaClient) {
