...
export function registerPracticeRoutes(app: FastifyInstance, prisma: PrismaClient) {
  const practiceService = createPracticeService(prisma);

  // POST /api/practice/generate
  app.post('/api/practice/generate', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.get('/api/practice/sets', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
    const userId = request.userId!;
    try {
  ...
  app.get('/api/practice/sets/:id', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/api/practice/sets/:id/attempt', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
    const { id } = request.params as { id: string };
    const userId = request.userId!;
  ...
  app.get('/api/practice/recommended', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
    const userId = request.userId!;
