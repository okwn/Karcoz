...
export function registerTelegramRoutes(app: FastifyInstance, prisma: PrismaClient) {
  ...
  app.post('/api/telegram/link-account', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  app.post('/api/telegram/unlink-account', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
    const requestId = generateRequestId();
    const body = request.body as { telegramChatId?: string };
  ...
  app.get('/api/telegram/status', { preHandler: requireSession }, async (request: FastifyRequest, reply: FastifyReply) => {
    const userId = (request.headers['x-user-id'] as string) || 'anonymous';
  ...
  app.post('/api/telegram/send-solution', async (request: FastifyRequest, reply: FastifyReply) => {
    const requestId = generateRequestId();
  ...
  app.post('/api/telegram/webhook', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (body.message) {
      ...
      const photo = msg.photo as unknown[] | undefined;
      ...
      if (photo && photo.length > 0) {
        const fileId = (photo[photo.length - 1] as Record<string, unknown>).file_id;
        // For a real bot we'd download the photo — here we just acknowledge
        await sendTelegramMessage(chatId,
          '📷 Image received! I\'ll solve this question for you.\n\n' +
        ...
        await telegramService.handleIncomingImage(chatId, fileId as string, caption);
      }
    }
