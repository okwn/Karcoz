...
export function registerScanRoutes(app) {
    // ── POST /api/scan/page-candidates ──────────────────────────────────────────
    app.post('/api/scan/page-candidates', async (request, reply) => {
...
function detectCandidatesFromText(pageText, pageTitle) {
    const candidates = [];
...
function calculateQuestionScore(text) {
    let score = 0;
    const lower = text.toLowerCase();
    if (/\b(what|how|why|when|where|who|which|whose|whom)\b/i.test(text))
        score += 0.3;
    if (/\b(solve|find|calculate|determine|explain|describe|compare|contrast|evaluate|identify)\b/i.test(text))
        score += 0.2;
    if (/\?\s*$/.test(text))
        score += 0.25;
    if (/\n\s*[A-Z]\.\s+/m.test(text) || /\n\s*\d+\.\s+/m.test(text))
    ...
    if (/[=<>+\-*/^]/.test(text) && /\d/.test(text))
        score += 0.1;
    if (text.trim().length >= 20 && text.trim().length <= 600)
        score += 0.1;
