...
export function registerBillingRoutes(app, prisma) {
    ...
    app.post('/api/billing/checkout', { preHandler: requireSession }, async (req, reply) => {
        ...
        try {
            const session = await billingService.createCheckoutSession(userId, plan, successUrl ?? '/settings?billing=success', cancelUrl ?? '/settings?billing=canceled');
