...
export function registerQuestionRoutes(app, prisma) {
    // ── GET /api/questions/history ─────────────────────────────────────────────
    app.get('/api/questions/history', { preHandler: requireSession }, async (request, reply) => {
        ...
        if (min_confidence !== undefined)
            where.confidenceScore = { gte: min_confidence };
    ...
    app.get('/api/questions/:id', { preHandler: requireSession }, async (request, reply) => {
    ...
    app.post('/api/questions/:id/save', { preHandler: requireSession }, async (request, reply) => {
        const { id } = request.params;
    ...
    app.delete('/api/questions/:id', { preHandler: requireSession }, async (request, reply) => {
        const { id } = request.params;
        const userId = request.userId;
        const question = await prisma.question.findFirst({ where: { id, userId } });
        if (!question) {
    ...
    app.get('/api/analytics/weak-topics', { preHandler: requireSession }, async (request, reply) => {
        const userId = request.userId;
    ...
    app.get('/api/analytics/overview', { preHandler: requireSession }, async (request, reply) => {
        const userId = request.userId;
      ...
      `,
        ...
        const avgConfResult = await prisma.$queryRaw `
    ...
    app.delete('/api/user/data', { preHandler: requireSession }, async (request, reply) => {
        const query = request.query;
